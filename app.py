...
# ─────────────────────────────────────────────
# FEATURES & PRECOMPUTED MODEL OUTPUTS
# ─────────────────────────────────────────────
# IMPORTANT FOR DEPLOYMENT:
# The model is NOT trained inside Streamlit Cloud.
# Streamlit only reads precomputed predictions from streamlit_predictions.csv.
# This keeps the original dashboard design/content but avoids startup crashes.
...
PREDICTIONS_PATH = "streamlit_predictions.csv"
...
def load_predictions():
    ...
    required = ["year", "governorate", "production", "baseline_prediction", "xgb_prediction", "final_prediction"]
    missing = [c for c in required if c not in pred.columns]
    if missing:
        raise ValueError(f"Missing required columns in streamlit_predictions.csv: {missing}")

    if "absolute_error" not in pred.columns:
        pred["absolute_error"] = abs(pred["production"] - pred["final_prediction"])
    if "ape_percent" not in pred.columns:
        pred["ape_percent"] = pred["absolute_error"] / np.maximum(abs(pred["production"]), 1) * 100
    if "signed_error" not in pred.columns:
        pred["signed_error"] = pred["final_prediction"] - pred["production"]
...
# FEATURE IMPORTANCE
# ─────────────────────────────────────────────
# Optional: if feature_importance.csv exists, the original feature-importance chart will appear.
# If not, the app simply skips that chart without breaking deployment.
@st.cache_data
def get_feature_importance():
    try:
        fi = pd.read_csv("feature_importance.csv")
        if {"feature", "importance"}.issubset(fi.columns):
            return fi[["feature", "importance"]].sort_values("importance", ascending=False).head(20)
    except Exception:
        pass
...
with fc3:
    st.markdown('<div class="filter-label" style="color:#7A8270;">⚠️ Show Govs with MAPE Above (%)</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    for _, row in year_map_df.iterrows():
        ...
        <div style="font-family:'DM Sans',sans-serif;padding:12px;min-width:220px;border-top:3px solid {color}">
            ...
            <div style="color:#555;font-size:12.5px;line-height:2.1;">
                ...
                <b>MAPE:</b> <span style="color:{color};font-weight:700">{ape:.1f}%</span>
            </div>
        </div>
        """
        folium.CircleMarker(
            ...
            tooltip=f"{nice_name(row['governorate'])}  |  MAPE: {ape:.1f}%  {icon}  {direction}",
    ...
    fig_map_bar.update_layout(
        title=f"MAPE by Governorate — {selected_year} (sorted worst → best)",
        yaxis_title="MAPE (%)"
    )
...
with tab3:
    ...
    if not one.empty:
        ...
        <div class="kpi-grid">
            ...
            <div class="kpi-card {ape_class}">
                ...
                <div class="kpi-label">MAPE This Case</div>
        ...
        with col_bar_g:
            ...
            gap_label    = f"{arrow_symbol} {abs(signed):,.0f} t gap · MAPE = {ape:.1f}%"
        ...
        with col_needle:
            fig_g = go.Figure(go.Indicator(
                ...
                title={"text": f"MAPE — {gov_display_name}", "font": {"size": 12, "family": "DM Sans", "color": C_MUTED}},
    ...
    if not one.empty:
        fig_trend.add_annotation(
            ...
            text=f"◀ {selected_year}<br>Actual: {fmt(actual)}<br>MAPE: {ape:.1f}%",
    ...
    display_cols = {
        ...
        "final_prediction": "Predicted (t)", "ape_percent": "MAPE (%)",
...
with tab6:
    ...
    with col_g2:
        ...
        fig_quad = px.scatter(
            ...
            labels={"production": "Actual Production (t)", "ape_percent": "MAPE (%)"}
        )
    ...
    fig_box.update_layout(
        ...
        yaxis_title="MAPE (%)", showlegend=False
    )
    ...
    gov_display2.columns = ["Governorate", "Avg Actual (t)", "Avg Predicted (t)", "MAPE (%)", "MAE (t)", "Years", "Status"]
