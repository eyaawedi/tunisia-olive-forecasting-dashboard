...
def load_data():
    ...
    # Force every non-categorical column to numeric.
    # This is important on Streamlit Cloud because CSV columns can sometimes be read as text.
    ...
    # SimpleImputer can handle NaN, but it cannot handle infinity.
    ...
    df["production_lag1"] = pd.to_numeric(df["production_lag1"], errors="coerce")
    df = df.dropna(subset=["production", "production_lag1"]).copy()
...
def train_model(df, feature_cols):
    ...
    # Keep only usable training rows
    train_df["production"] = pd.to_numeric(train_df["production"], errors="coerce")
    train_df = train_df.replace([np.inf, -np.inf], np.nan)
    train_df = train_df.dropna(subset=["production"]).copy()

    X_train = train_df[feature_cols].copy()
    y_train = train_df["production"].astype(float).copy()

    # Force categorical/numeric split manually to avoid cloud dtype issues
    cats = ["governorate"] if "governorate" in X_train.columns else []
    ...
    # If a numeric feature is fully missing in training, remove it safely
    usable_nums = [c for c in nums if not X_train[c].isna().all()]
    cats = [c for c in cats if c in X_train.columns]

    transformers = []
    if usable_nums:
        transformers.append(("num", Pipeline([("imp", SimpleImputer(strategy="median"))]), usable_nums))
    if cats:
...
def get_feature_importance():
    try:
        ...
        return pd.DataFrame({"feature": names, "importance": imp}).sort_values("importance", ascending=False).head(20)
    except Exception:
        return pd.DataFrame()
